...
const StrikeRateZones = ({ playerName }) => {
  ...
  return (
    <div className="w-full">
      <div className="mb-4">
        <h4 className="text-lg font-semibold text-white mb-2">Strike Rate Zones</h4>
        {/* Player Note */}
        <div className="bg-teal-600 text-white px-3 py-1 rounded-md text-sm font-medium inline-block">
      ...
      <div className="relative rounded-lg p-4 overflow-hidden" style={{ background: 'linear-gradient(180deg, #9B8B7A 0%, #8B7355 50%, #9B8B7A 100%)' }}>
        ...
        <div className="relative z-10">
          {/* Header Row */}
          <div className="grid grid-cols-4 gap-1 mb-2 max-w-lg mx-auto">
            <div></div> {/* Empty space for length labels */}
            <div className="text-center text-sm font-semibold text-white py-2">Off</div>
            <div className="text-center text-sm font-semibold text-white py-2">Line</div>
            <div className="text-center text-sm font-semibold text-white py-2">Leg</div>
          </div>
            ...
            {['bouncer', 'short', 'back of length', 'length', 'full', 'yorker'].map((length, rowIndex) => (
              <div key={length} className="mb-1">
                <div className="grid grid-cols-4 gap-1 items-center">
                  ...
                  <div className="text-left text-sm font-semibold text-white py-2">
                  ...
                  {['off', 'line', 'leg'].map((line) => {
                    ...
                    return (
                      <div
                        key={`${line}-${length}`}
                        className={`
                          flex items-center justify-center py-2 px-1 rounded
      ...
      {/* Color Legend */}
      <div className="mt-4">
        <h5 className="text-sm font-semibold text-white mb-2">Performance Legend</h5>
